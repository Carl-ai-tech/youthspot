...
18–35 歲年死亡率約 0.05%，一年 1,000 人不到 1 人，忽略。

實測（新北市 2025/7 → 2026/7）：淡水區「人口差」−275，世代淨遷入 **+1,069**；
全市人口差 −21,755，世代淨遷入 +546 —— 人口差幾乎全是少子化，跟青年搬不搬走無關。
用人口差配資源會把錢投錯區。

資料：每年 7 月一期，戶政司從 107 年 7 月（2018）起有，所以序列是 2019–2026 共 8 點。
...
FIRST_PERIOD = "10707"          # 戶政司這個資料集從 107 年 7 月起有
AGE_LO, AGE_HI = 18, 35
...
def fetch_migration(region: str = "新北市", *, refresh: bool = False, period: str | None = None) -> dict:
    ...
    return {"source": DATASET, "url": LANDING, "region": region, "years": years,
            ...
            "note": ("世代追蹤：今年 a 歲 − 去年 a−1 歲，Σ 18–35 歲。扣掉了世代縮小與（可忽略的）死亡，"
                     "剩下的就是淨遷徙。戶籍資料，量的是把戶籍遷來的青年，不含未遷籍的就學就業者。")}
