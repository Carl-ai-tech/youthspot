    ...
    if ref:
        ...
        if subject is not None:
            ...
            hint = (f"這題問的是「下一個」：可供驗證的候選來自「{region}內條件最像 {ref['short']}」清單裡挑"
                    ...
                    + (f" 並提醒：{ref['short']} 實際移入比這四個條件解釋的高 {ref['resid']:+.1f} 個百分點，多出來的差異原因未知；住宅供給與交通是可調查方向 —— 住宅供給沒有行政區層級的官方開放資料（建照只到縣市）；交通只放得進「有無軌道站」（模型 D，方向正但不顯著），通勤時間沒有行政區資料 —— 不能稱為已知或最可能原因。條件像仍需要後續資料確認。" if ref.get("resid") is not None else
                       " 並提醒：條件像只是假說線索，不是必要或充分條件；住宅供給與交通是可調查方向（住宅供給沒有行政區層級的官方開放資料（建照只到縣市）；交通只放得進「有無軌道站」（模型 D，方向正但不顯著），通勤時間沒有行政區資料），不能稱為已知原因。"))
    return lines, hint
