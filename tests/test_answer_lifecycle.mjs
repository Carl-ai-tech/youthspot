...
el('askInput').value='新北市 25-29 歲的平均年薪';fire('askInput','input');sandbox.__ask();
const local = el('askText').textContent;
await settle(0,'STALE_OLD_ANSWER');assert.equal(el('askText').textContent,local);
el('askInput').value='請分析臺北市青年政策';fire('askInput','input');sandbox.__ask();
assert.equal(fetchCalls[1].body.region,'臺北市','Explicit city overrides selected district');
el('askInput').value='請分析新北市青年政策';fire('askInput','input');sandbox.__ask();
await settle(2,'LATEST_ANSWER');const newest=el('askText').innerHTML;
await settle(1,'STALE_SECOND_ANSWER');assert.equal(el('askText').innerHTML,newest);
assert.ok(newest.includes('LATEST_ANSWER'));
fire('synthBtn','click');assert.equal(fetchCalls[3].body.action,'synthesize');
el('area').value='新北市林口區';fire('area','change');
await settle(3,'STALE_SCOPE');assert.ok(!el('askText').innerHTML.includes('STALE_SCOPE'));
el('askInput').value='請分析失敗時青年政策';fire('askInput','input');sandbox.__ask();
queue[4].reject(new Error('network failed'));await new Promise(r=>setImmediate(r));
...
console.log('PASS request deduplication, selected and explicit regions, stale responses, shared synthesis lifecycle, scope change and recovery');
el('askInput').value='請分析新北市板橋區的青年政策';fire('askInput','input');sandbox.__ask();
assert.equal(fetchCalls[5].body.region,'新北市板橋區','Full district name must outrank its city prefix');
await settle(5,'DISTRICT_ANSWER');
console.log('PASS explicit full district name outranks city prefix');
